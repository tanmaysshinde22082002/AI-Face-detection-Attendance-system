    ...
    if request.method == 'POST':
        ...
        if studentForm.is_valid() and (stat == False):
            ...
            messages.error(request,
                           'Student with Registration Id ' + request.POST['registration_id'] + ' already exists.')
            return redirect('home')
...
def takeAttendence(request):
    if request.method == 'POST':
        ...
        if Attendence.objects.filter(date=str(date.today()), branch=details['branch'], year=details['year'],
                                     section=details['section'], period=details['period']).count() != 0:
